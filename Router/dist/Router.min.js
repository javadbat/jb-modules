...
function () {
  ...
  */
  ...
  function Router(config) {
    ...
    _defineProperty(this, "treeBaseRoutes", []);

    _defineProperty(this, "routes", []);

    _defineProperty(this, "routeGroupsName", []);

    _defineProperty(this, "basePath", void 0);

    _defineProperty(this, "matchedRoute", {});

    _defineProperty(this, "componentsPathPrefix", '/');

    _defineProperty(this, "pageContainerTagId", void 0);

    _defineProperty(this, "isPageContainerMounted", false);
    ...
    _defineProperty(this, "pagesTitlePrefix", "");

    _defineProperty(this, "loadState", void 0);
    ...
    window.addEventListener('popstate', function (e) {
      return _this.onBrowserPopState(e);
    });
    return instance[this.config.instanceName];
  }
  ...
  _createClass(Router, [{
    key: "initLoadState",
    value: function initLoadState() {
      this[_loadStateKey] = null;
    }
  }, {
    key: "callLoadStateChange",
    value: function callLoadStateChange(oldValue, newValue) {
      if (this.config.events.onLoadStateChange) {
        this.config.events.onLoadStateChange(oldValue, newValue);
      }
    }
  }, {
    key: "onBrowserPopState",
    value: function onBrowserPopState(event) {
    ...
    key: "normalizeAndOptimazeRoutes",
    value: function normalizeAndOptimazeRoutes() {
      var _this2 = this;
      ...
      var treeRouteTraverse = function treeRouteTraverse(routes, urlPrefix, settings) {
        ...
        try {
          for (var _iterator = routes[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
            ...
            route['params'] = {}; //Add Route Group Name

            route['settings'] = Object.assign(route.settings || {}, settings); //Add UrlPrefix

            route['url'] = urlPrefix + route['url']; //add $ symbol make regx to detect exact url (make it equal not include)

            route['regex'] = new RegExp(route.url.concat("$").toLowerCase().replace(/\{\w+\}/g, '([\\w-]+)')); //Get Param Of each Route
    ...
    key: "matchPathToOneRoute",
    value: function matchPathToOneRoute(normalizedPath) {
    ...
    key: "loadReactComponent",
    value: function loadReactComponent(reactComponentPath) {
      var _this3 = this;

      //Get Components
      SystemJS.import(this.componentsPathPrefix + reactComponentPath).then(function (componentClass) {
        _this3.callLoadStateChange(_this3[_loadStateKey], 1);

        _this3[_loadStateKey] = 1;
        ...
        if (componentClass.default == undefined || componentClass.default == null) {
          ...
          ReactDOM.unmountComponentAtNode(_this3.containerDom); //initiate react component to its place in page

          ReactDOM.render(React.createElement(componentClass.default, null), _this3.containerDom, _this3.onPageLoadComplete(componentClass));
        }
      }).catch(function (e) {
        ExceptionHandler.newException(e, "در هنگام لود کامپوننت مشکلی پیش آمده است");

        if (!_this3.loadComponentTryCount) {
          _this3.loadComponentTryCount = 1;
        } else {
          _this3.loadComponentTryCount++;
        }

        if (_this3.loadComponentTryCount < 6) {
          //try to load it again 
          setTimeout(function () {
            _this3.loadReactComponent(reactComponentPath);
          }, _this3.loadComponentTryCount * 2000);
        } else {
          //when we disapoint to try to load again
          //we show 424 error page
          _this3.loadReactComponent(_this3.config.standardPageRoutes[424].reactComponentPath);
        }
      });
    }
    /**
    ...
    key: "setPageAttributes",
    value: function setPageAttributes(title, absolutePath) {
    ...
    value: function onPageLoadComplete(jsClass) {
